...
//		refill.addActionListener(new SignalClient(Ports.PORT_LOADER_PLANT, Ports.REFILL_SIGNAL));
		JButton grip = new JButton("enable");
		grip.addActionListener(new SignalClient(Ports.PORT_LOADER_PLANT, Ports.GRIP_SIGNAL));
		JButton lower = new JButton("bottle ready");
		lower.addActionListener(new SignalClient(Ports.PORT_LOADER_PLANT, Ports.LOWER_SIGNAL));
...
//		ad.addItemListener(new SignalCheckBoxClient(Ports.PORT_LOADER_CONTROLLER, Ports.SIGNAL_ARM_DEST));
		JCheckBox ze = new JCheckBox("gripperZAxisLoweredM");
		ze.setEnabled(false);
		ze.addItemListener(new SignalCheckBoxClient(Ports.PORT_LOADER_CONTROLLER, Ports.SIGNAL_ZAXISEXTENDED));
		JCheckBox tr = new JCheckBox("gripperTurnHomeM");
		tr.setEnabled(false);
		tr.addItemListener(new SignalCheckBoxClient(Ports.PORT_LOADER_CONTROLLER, Ports.SIGNAL_GRIPPERTURNRETRACT));
		JCheckBox zl = new JCheckBox("gripperZAxisLiftedM");
		ze.setEnabled(false);
		ze.addItemListener(new SignalCheckBoxClient(Ports.PORT_LOADER_CONTROLLER, Ports.SIGNAL_ZAXISEXTENDED));
		JCheckBox tf = new JCheckBox("gripperTurnFinalM");
		tr.setEnabled(false);
		tr.addItemListener(new SignalCheckBoxClient(Ports.PORT_LOADER_CONTROLLER, Ports.SIGNAL_GRIPPERTURNRETRACT));
