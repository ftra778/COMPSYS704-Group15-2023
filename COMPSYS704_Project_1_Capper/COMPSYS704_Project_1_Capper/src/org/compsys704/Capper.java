...
public class Capper extends JFrame {
	...
	public Capper() {
		...
		enable.addActionListener(new SignalClient(Ports.PORT_CAPPER_PLANT, Ports.ENABLE_SIGNAL));
		JButton bottleReady = new JButton("Bottle Ready");
		bottleReady.addActionListener(new SignalClient(Ports.PORT_CAPPER_CONTROLLER, Ports.BOTTLE_READY_SIGNAL));
		...
		// Radio buttons
		SignalRadioClient src = new SignalRadioClient(Ports.PORT_CAPPER_CONTROLLER, Ports.SIGNAL_Mode);
		...
		cb.setEnabled(false);
		cb.addItemListener(new SignalCheckBoxClient(Ports.PORT_CAPPER_CONTROLLER, Ports.SIGNAL_CLAMP_BOTTLE));
		JCheckBox gc = new JCheckBox("gripCap");
		gc.setEnabled(false);
		gc.addItemListener(new SignalCheckBoxClient(Ports.PORT_CAPPER_CONTROLLER, Ports.SIGNAL_GRIP_CAP));
		JCheckBox lc = new JCheckBox("lowerCapper");
		lc.setEnabled(false);
		lc.addItemListener(new SignalCheckBoxClient(Ports.PORT_CAPPER_CONTROLLER, Ports.SIGNAL_LOWER_CAPPER));
		JCheckBox th = new JCheckBox("rotateGripperHome");
		th.setEnabled(false);
		th.addItemListener(new SignalCheckBoxClient(Ports.PORT_CAPPER_CONTROLLER, Ports.SIGNAL_GRIPPER_TURN_HOME));
		JCheckBox tf = new JCheckBox("rotateGripperFinal");
		tf.setEnabled(false);
		tf.addItemListener(new SignalCheckBoxClient(Ports.PORT_CAPPER_CONTROLLER, Ports.SIGNAL_GRIPPER_TURN_FINAL));
